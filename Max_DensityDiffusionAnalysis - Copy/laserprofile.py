...
##### correcting data
def correct_data(files, profile_file, roi_coordinates=None, mask_num=None, mask_bool=False, fig_bool=False, smoothing_factor=4, thresh=200):
    '''
    Parameters:
    ...
    elif (mask_bool == False) and (mask_num != None):
    ...
    if filename ==  '':
        ...
        ###### create roi object for cropping
            ...
            with pims.open(cur) as seq:
                ...
                if mask_num is not None and mask_bool==True:
                    ...
                    for s in range(mask_num,length):
                        corrected_image = (r(seq[s]))/profile_mask
                        masking = cluster(corrected_image, thresh=thresh)
