...
def filter_out_statics(trc_data, thresh):
    '''
    Filters out tracks with an average movement below a given threshold:
        trc_data: dictionary with cell specification as values ('c1', 'c2', etc) and list of dataframes with tracked data
        thresh: threshold of movement below which the track is filtered out
    returns:
        trc_data_new: dictionary trc_data with the tracks with movement below thresh filtered out
        distance2_histogram: array of the variance of the displacement of each track (can be used to tune thresh)
    '''
    trc_data_new = []
    distance2_histogram = []
