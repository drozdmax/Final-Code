...
def get_mean_profile(filename, profile_path, profile_frame=0):
    ...
    fig.savefig(profile_path.format('profile_mean.svg'))
...
def get_std_profile(filename, profile_path, profile_frame=0):
    ...
    fig.savefig(profile_path.format('profile_std.svg'))
...
def get_fit_mask(data, profile_path, filename, fig_show=True):
    ...
    plt.savefig(profile_path.format(filename[:-4]+'_fit_params.svg'), bbox_inches='tight')
    ...
    if fig_show == True:
        ...
        fig.savefig(profile_path.format(filename[:-4]+'_fit.svg'), bbox_inches='tight')
    ...
    if fig_show == True:
        ...
        fig.savefig(profile_path.format(filename[:-4]+'_norm_fit.svg'), bbox_inches='tight')
...
def correct_data(files, profile_file, mask_num=None, mask_bool=False, mask_edges=False, fig_bool=False, smoothing_factor=4, thresh=200):
    ...
    if filename ==  '':
        ...
        if fig_bool == True:
            ...
            fig.savefig(profile_path.format(filename[:-4] +'_gaussian_smoothing.svg'), bbox_inches='tight')
