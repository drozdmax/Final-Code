...
def colocalization_analysis(analysis_folder, analysis_frame, dimensions, correction_registrator, split_axis=1, set_roi=None, d_coloc=1, subsequent_recording=False, correct_random_coloc=False):
    """
    A colocalization analysis is done for localizations present in a given analysis frame for a set of measurements, a sdt.multicolor.Registrator object for coordinate correction is required as input. Returned are the number of localizations in each channel, and the number of colocalizations.
    ...
    Returns:
        n_1: number of localizations in channel 1
        n_2: number of localizations in channel 2
        n_coloc: number of calculated colocalizations
    """
    h5_files = getfiles(extension='h5', folder=analysis_folder)
    ...
    if correct_random_coloc:
        n_coloc_random = 0
        loc_r2_mirror = copy.deepcopy(loc_r2)
        x_temp = []
        y_temp = []
        
        for loc_temp in loc_r2_mirror:
            x_temp.append(loc_temp['x'])
            y_temp.append(loc_temp['y'])
        x_mean = np.mean(np.concatenate(x_temp))
        y_mean = np.mean(np.concatenate(y_temp))
        
        for loc_temp in loc_r2_mirror:
            loc_temp['x'] = 2.0*x_mean-loc_temp['x']
            loc_temp['y'] = 2.0*y_mean-loc_temp['y']
        
        for loc_r1_temp, loc_r2_temp in zip(loc_r1, loc_r2_mirror):
            colocs = multicolor.find_colocalizations(loc_r1_temp, loc_r2_temp, max_dist=d_coloc)
            n_coloc_random += len(colocs)
    ...
    if correct_random_coloc:
        n_coloc -= n_coloc_random
    
    return n_1, n_2, n_coloc


def coloc_dimer_fraction(n_1, n_2, n_coloc):
    """
    Returned are the calculated dimer fraction, assuming a population of monomers and dimers with complete labeling and no partial photobleaching, and the fraction of fluorophores belonging to the first channels wavelength.
...
def mle_score(analysis_data_photons, monomer_data_photons, lim=1000, sigma_factor=1.0, fit_lognorm=False, savepath=None, donotplot=False):
    '''
    Creates probabilty density functions and return the maximum scores of the maximum likelihood estimator for fitting up to 2-mers. The score is plotted for fractions between 0 and 1. The result is returned for both random addition (RA) and multiplied distributions (MD) assumptions for the summation of fluorophore brightness values.
    ...
    ####### Get results using all data
    ### Generate PDFs
    pdfs_MD, pdf_data_MD = get_pdfs_local(analysis_data_photons, monomer_data_photons, lim=lim, lim_con=2, sigma_factor=sigma_factor, fit_lognorm=fit_lognorm, method='MD')
    
    # Generate model function for the PDF
    model_pdf_MD = lambda x, alphas: interpolate.interp1d(pdf_data_MD[0], model_pdf(pdfs_MD, *alphas), kind='cubic')(x[x<=lim])
    # Initializing the negative log of the likelihood function to be minimized
    mle_score_MD = lambda alphas: 1.0*np.sum(np.log(model_pdf_MD(analysis_data_photons, alphas)))    
    
    ### Generate PDFs
    pdfs_RA, pdf_data_RA = get_pdfs_local(analysis_data_photons, monomer_data_photons, lim=lim, lim_con=2, sigma_factor=sigma_factor, fit_lognorm=fit_lognorm, method='RA')
    
    # Generate model function for the PDF
    model_pdf_RA = lambda x, alphas: interpolate.interp1d(pdf_data_RA[0], model_pdf(pdfs_RA, *alphas), kind='cubic')(x[x<=lim])
    # Initializing the negative log of the likelihood function to be minimized
    mle_score_RA = lambda alphas: 1.0*np.sum(np.log(model_pdf_RA(analysis_data_photons, alphas)))
    
    alpha_vals = np.linspace(0, 1, 200)
    
    mle_scores_MD = np.zeros_like(alpha_vals)
    mle_scores_RA = np.zeros_like(alpha_vals)
    for i in range(len(alpha_vals)):
        mle_scores_MD[i] = mle_score_MD([alpha_vals[i]])
        mle_scores_RA[i] = mle_score_RA([alpha_vals[i]])
    
    max_score_MD = np.max(mle_scores_MD)
    max_score_RA = np.max(mle_scores_RA)
    
    if donotplot:
        return min_score_MD, min_score_RA
    
    plt.rcParams.update({'font.size': 13})
    fig, ax = plt.subplots(constrained_layout=True)
    
    ax.plot(alpha_vals, mle_scores_MD, label='MD Score')
    ax.plot(alpha_vals, mle_scores_RA, label='RA Score')
    ax.legend()
    ...
    return max_score_MD, max_score_RA